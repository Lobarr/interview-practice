...
class PriorityQueue:
  ...
  def swap(self, fromIndex: int, toIndex: int):
    self.queue[fromIndex], self.queue[toIndex] = self.queue[toIndex], self.queue[fromIndex]
    
  def verifyNode(self, node: PNode):
    return (
      0 <= node.getIndex() < self.getCount() 
  ...
  def bubble(self, index):
    if (
      index > 0
      and index <= self.getCount() - 1
      and self.queue[index] < self.queue[self.getParentIndex(index)]
    ):
      self.bubbleUp(index)
    else:
  ...
  def bubbleDown(self, index):
    if 0 < index < self.getCount() - 1:
      ...
      if (
        rightChildNodeIndex != None
        and self.queue[rightChildNodeIndex] < self.queue[leftChildNodeIndex]
      ):
