...
class Board(ABC):
    ...
    def __init__(self, rowSize: int, colSize: int):
        self.board = [[Board.COORDINATE_PLACEHOLDER] * colSize
    ...
    def validateCoordinate(self, x: int, y: int) -> bool:
        if (0 <= x < self.rowSize and 0 <= y < self.colSize):
            return True
        return False
    ...
    def isFull(self) -> bool:
        for row in self.board:
            if len(
                    list(
                        filter(lambda x: x == Board.COORDINATE_PLACEHOLDER,
                               row))) > 0:
                return False
...
class TicTacToeBoard(Board):
    ...
    def checkWin(self, player: str) -> bool:
        if (self.checkHorizontal(player) or self.checkVertical(player)
